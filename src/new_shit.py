...
def disp_trajectory(plan):
    global display_trajectory_pub  # Declare the variable as global

    display_trajectory = moveit_msgs.msg.DisplayTrajectory()

    if isinstance(plan, tuple):
        # Handle the case when the plan is a tuple (failed plan)
...
def go_to_pose_goal(fixed_pose):
    ...
    arm_group.set_pose_target(fixed_pose)
...
def pc_callback(cloud_msg):
    ...
    # o3d_cloud.estimate_normals(search_param=o3d.geometry.KDTreeSearchParamKNN(knn=20))

    # # Get the normals as a numpy array
    # normals = np.asarray(o3d_cloud.normals)
    # points = np.asarray(o3d_cloud.points)

    # waypoints = visualize_normals(points, normals)
